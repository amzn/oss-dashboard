...
class UnknownMembersDbReporter < DbReporter
  ...
  def db_report(context, org, sync_db)
    unknown=sync_db["SELECT DISTINCT(m.login) FROM member m, repository r, team_to_member ttm, team_to_repository ttr WHERE m.login NOT IN (SELECT login FROM users) AND m.id=ttm.member_id AND ttm.team_id=ttr.team_id AND ttr.repository_id=r.id AND r.org=?", [org]]
    text = ''
    unknown.each do |row|
