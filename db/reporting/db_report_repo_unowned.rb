...
class RepoUnownedDbReporter < DbReporter
  ...
  def db_report(context, org, sync_db)
    ...
    # repos with no team
    noteam=sync_db.execute("SELECT r.name, r.created_at FROM repository r WHERE r.id NOT IN (SELECT DISTINCT repository_id FROM team_to_repository) AND r.org=?", [org])

    text = ''
    emptyteam.concat(noteam).each do |row|
      text << "  <reporting class='repo-report' repo='#{org}/#{row[0]}' type='RepoUnownedDbReporter'><field>#{row[1]}</field><field>#{org}/#{row[0]}</field></reporting>\n"
    end
